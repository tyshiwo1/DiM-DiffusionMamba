...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # # flash attn , qkv = einops.rearrange(qkv, 'B L ( H D) ->  B H L D', K=3, H=self.num_heads).float()
        # b,c,h,w = q.shape
        # q = rearrange(q, 'b c h w -> b (h w) c').unsqueeze(1)
        # k = rearrange(k, 'b c h w -> b (h w) c').unsqueeze(1)
        # v = rearrange(v, 'b c h w -> b (h w) c').unsqueeze(1)
        # h_ = F.scaled_dot_product_attention(q, k, v)
        # h_ = rearrange(h_, 'b t (h w) c -> b (t c) h w', h=h, w=w)
...
class FrozenAutoencoderKL(nn.Module):
    ...
    def encode_moments(self, x, chunk_size=25):
        # h = self.encoder(x)
        h = []
        for i in range(0, len(x), chunk_size):
            frame = x[i : i + chunk_size]
            frame = self.encoder(frame)
            h.append(frame)
    ...
    def decode(self, z, chunk_size=25):
        ...
        for i in range(0, len(z), chunk_size):
            frame = z[i : i + chunk_size]
            frame = self.decoder(frame)
            dec.append(frame)

        dec = torch.cat(dec, dim=0)
        return dec
